...
class Etcd3Client():
    def __init__(self, host, port, user, passwd, timeout=2000):
        self.__version__ = 'v3.4_1'
        self.host = host
        self.port = port
        self.user = user
        self.passwd = passwd
    ...
    def _get_range(self, key, range_end=None, limit=None, revision=None, sort_order=None, sort_target='key',
        ...
        try:
            ...
            print(serializable)
            print(count_only)
            print(min_mod_revision)
            print(max_mod_revision)
            print(min_create_revision)
            print(max_create_revision)
            print(json.dumps(params))
            print("----____")
            data = self.conn.post(url=url, headers=headers, data=json.dumps(params), timeout=self.timeout)
            print(data)
            print("---------------------")
            return data
        except Exception as e:
    ...
    def get(self, key, **kwargs):
        try:
            print(key)
            resp = self.get_response(key=key, **kwargs)
            if resp.status_code == 200:
                data = resp.json()
                if data['count'] < 1:
                    return (True, None)
                else:
                    return (True, self._debase64(data['kvs'][0]['value']))
            else:
                return (False, None)
        except Exception as e:
            self.error = str(e)
...
if __name__ == '__main__':
    client = Etcd3Client(host='192.168.226.130',port='2379',user='',passwd='')
    client.Connect()
    # status = client.put(key='a',value='b1')
    # print(status)
    value = client.get('a')
    print(value)
