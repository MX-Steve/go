...
func handleSetMachine(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	mac := ps.ByName("mac")
	ip := ps.ByName("ip")
	log.Infof("One machine connected, mac address:%s, ip address:%s", mac, ip)
	...
	if !ok {
		...
		err := writeFile(mac, newPort, ip, true)
		if err != nil {
			log.Fatal(err)
		}
		...
		if ip != machine.IP {
			log.Infof("Update machine %s ip from:%s to :%s", mac, machine.IP, ip)
			err := writeFile(mac, machine.TunnelPort, ip, false)
			if err != nil {
				log.Fatal(err)
			}
		}
...
func writeFile(mac, port, ip string, isNew bool) error {
	mutex.Lock()
	defer mutex.Unlock()
